...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 5 ms | 1 ms | 1 ms | 1 ms | 1 ms | 1 ms | n/a |
| 100 | 18 ms | 6 ms | 14 ms | 11 ms | 10 ms | 307 ms | 2.84x |
| 1,000 | 90 ms | 326 ms | 102 ms | 688 ms | 659 ms | 3.7 s | 2.6x |

Initial manual budgets for the current implementation on a stable local or dedicated runner are:

- 1,000-record cold hydrate and stale prepare should remain below 250 ms.
- 1,000-record unchanged prepare, list, and show should remain below 50 ms.
- 1,000-record compact and full search should remain below 50 ms for the benchmark query.
- 1,000-record gather with two broad searches should remain below 5 seconds.
...
The v0.1 full-rebuild cache is suitable for small and medium repository knowledge bases up to the product corpus limit of 1,000 canonical records. On the baseline machine, cold hydration and stale prepare stayed near 100 ms at that limit, and individual list/show/search reads remained interactive.
