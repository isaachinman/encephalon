...
- Review remediation reproduced and fixed two independent failures: maximum NFC expansion exceeded the old derived FTS bounds, and an exclusively created cache primary disappearing before inspection bypassed the existing creation-conflict retry. Both focused regressions and their deliberate mutations pass/fail as expected.
- Lint, all four TypeScript projects, the full suite (514 pass, 0 fail, 2 established capability skips), both benchmarks, build, packed-package Unicode CLI validation, publish contract, frozen install, declarations, Bun/config, and diff-hygiene gates pass after review remediation at `ec1360a8c03e9d9b2ab12562f6ba31dc697c2f09`.
