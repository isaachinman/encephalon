...
2. Removing runner credential detection would allow `${{ secrets.TOKEN }}` or effective `id-token: write` without an environment; treating reusable callers as runners would demand an unsupported caller environment.
3. Allowing external reusable-workflow callers to forward named or inherited secrets would move credentials outside the recursively inspected repository boundary.
4. Broadening permissions would allow `contents: write` or OIDC outside the protected Pullfrog job.
5. Removing containment or cycle handling would allow `./../outside/action` or a self-referencing local action to escape or loop.
6. Removing deterministic sorting would make diagnostics depend on directory or object insertion order.
