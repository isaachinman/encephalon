...
- External owner/repository[/path] references must end in one lowercase 40-character commit SHA; a trailing comment records the human-readable release. Executable `docker://` action references must end in `@sha256:` plus 64 lowercase hex characters, including reachable local Docker actions' `runs.image`. GitHub-runner-recognised repository-relative Dockerfile images whose basename starts `Dockerfile.` or ends `Dockerfile`, case-insensitively, remain accepted, but their `FROM` chains are outside policy proof.
...
- For every reachable local action with `runs.using: docker`, accept a string `runs.image` only when it is a GitHub-runner-recognised repository-relative Dockerfile whose basename starts `Dockerfile.` or ends `Dockerfile`, case-insensitively, or a full immutable `docker://` digest. Report every other string image at `runs.image` under `external-image-digest` without claiming that accepted Dockerfile `FROM` chains are inspected or proven.
