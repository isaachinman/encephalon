...
- Recursive source visits are keyed by comparable path and workflow/action role. Final acceptance revalidates repository/workflow discovery, every successfully read file witness, and every resolved action directory's exact `action.yml`/`action.yaml` candidate set; any late instability discards provisional findings and returns one deterministic `source-integrity` finding without retrying traversal.
...
5. Broadening permissions would allow `contents: write` or OIDC outside the protected Pullfrog job.
...
- Guard YAML parsing and emit a `source-integrity` finding for invalid discovery and unreadable, malformed, or unsafe source YAML rather than throwing raw parser diagnostics. Reserve `local-reference` for disallowed, escaping, or missing local `uses` targets.
- Traverse arrays and plain parsed objects without evaluating template expressions.
- Track each comparable absolute path and workflow/action role in a bounded visit set before following local references, allowing one source to be inspected once in each role while same-role cycles terminate.
...
- Retain each successful validated file read's final stable metadata/path witness and each unique resolved action directory's stable generation plus exact `action.yml`/`action.yaml` observations. At one optional-hooked final boundary, revalidate discovery and every retained witness; discard provisional findings and return one deterministic `source-integrity` finding on any mismatch without retrying or traversing again.
...
- Allow repository-local `$/` and `./` reusable-workflow callers to forward named secrets or use `secrets: inherit`; recursive inspection enforces the environment on actual credential-consuming runners in the called workflow. Reject any `secrets` property on an external reusable-workflow caller at `jobs.<id>.secrets` under `credential-forwarding`, while preserving existing external pinning checks.
...
- Task 1's structural policy and behavioural tests were committed at `16266f47db0c802d818d85db139472616aa930e7`. Task 2's core repository-controlled workflow, policy, configuration, and behavioural-test snapshot remains `a2bb85a7b28aefedbcb13b4c61d16bbce3f76c57`; that full SHA is the reviewed implementation provenance consumed by Task 3. Branch-review remediation and its behavioural tests were committed at `31ba01944a1e4afccf867c7f7607859c437f3e0e`. Cross-platform remediation at `a242d3f614775d24694ca7122c40efbbc6ec1a1a` made path comparison Windows-safe, used junction-aware fixtures, and normalised equivalent parser trigger keys. Workflow policy review at `6c54f0032be3fcf8932a8e1ba80b8e2bc6d5354a` added invalid-root diagnostics, inherited OIDC environment enforcement, exact pin/comment binding coverage, and native Windows case and separator coverage. Review remediation at `f22fafeec2346d1ced699433711dd51985a2f9e8` rejected multiply linked workflow inputs, made the POSIX-only fixture portable, and added documented `$/` self-repository references. Review remediation at `a8367f56ccab83755ef27e7636de2efc94f70538` made executable-reference traversal schema-aware, rejected workspace-relative action execution while preserving same-commit reusable-workflow compatibility, and added mechanical Bun declaration-boundary coverage; it is the preceding reviewed repository-controlled code and test state documented by this plan. Later caller-policy correction distinguishes reusable-workflow callers from runners, permits local credential forwarding only across recursively inspected calls, rejects external credential forwarding, and applies workflow-level secret env only to runners. Task A traversal remediation keys visits by source role, consolidates stable metadata and ordinal-order helpers, introduces the `source-integrity` taxonomy, and performs one final discovery/file/action-candidate witness revalidation before accepting findings.
- Fresh Task 3 workflow policy validation passed all 13 tests with zero failures, and the recursive policy CLI returned no findings. At the branch-review remediation head, workflow policy validation passed all 17 tests and the combined workflow/package run passed all 24 tests with zero failures. At the cross-platform remediation head, workflow policy validation passed all 18 tests and the combined workflow/package run passed all 25 tests with zero failures. At the final review-remediation head, the focused workflow-policy run passed all 22 applicable tests with zero failures and one host-inapplicable Windows-only test skip. Fresh documentation-head lint checked 117 files with no fixes, all four TypeScript projects passed, the full suite reported 564 tests with 562 passed, zero failed, and two established capability skips, and diff hygiene returned no findings. Pull-request CI run `32656889751` passed the complete Ubuntu current, Ubuntu latest, macOS latest, and Windows latest matrix at the earlier `f22fafeec2346d1ced699433711dd51985a2f9e8` code-and-test snapshot.
