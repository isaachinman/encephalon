...
- The exact current-main-integrated MAR-2574 code and behavioural-test implementation snapshot is `177320d368627c2bb6421afe73fafe4bad9de58c`. It preserves the current release-equivalent gate and trusted retained-artifact flow, runs direct workflow validation only after a frozen lifecycle-disabled install, and retains present, missing, and invalid contained Dockerfile observations through both final sweeps. The maintained contract and programme design carry this exact snapshot; the documentation-only provenance commit does not alter runtime or workflow behaviour.
- At `177320d368627c2bb6421afe73fafe4bad9de58c`, 67 focused workflow-policy tests passed with one host-inapplicable Windows-only skip and zero failures; the independent CI bootstrap suite added three passes. The repository policy CLI returned no findings, lint checked 141 files with no fixes, all four TypeScript projects passed, and the full suite reported 768 tests with 765 passed, three established capability skips, and zero failures. Frozen lifecycle-disabled installation checked 39 installs across 66 packages without changes. Benchmarks, build, generated-version validation, package validation, the expected already-published-version publish contract, Encephalon validation of 39 records, and diff hygiene all passed locally before this provenance-only commit.
