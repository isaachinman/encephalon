...
- External owner/repository[/path] references must end in one lowercase 40-character commit SHA; a trailing comment records the human-readable release. Executable `docker://` action references must end in `@sha256:` plus 64 lowercase hex characters, including reachable local Docker actions' `runs.image`; `runs.using` is matched case-insensitively like the GitHub runner. Existing repository-contained single-link regular Dockerfiles whose basename starts `Dockerfile.` or ends `Dockerfile`, case-insensitively, remain accepted and identity-bound through final revalidation, but their contents and `FROM` chains are outside policy proof.
...
- Repository-local reusable-workflow callers may forward named secrets or use `secrets: inherit` because the called workflow is recursively inspected and its credential-consuming runner jobs independently require `pullfrog-review`. Direct secret-context values in reusable-call `with` inputs are rejected, forcing local credentials through `secrets`; external reusable workflows remain subject to immutable pinning, require exact `jobs.<id>.permissions: {}`, and must omit both `secrets` and direct secret-context inputs. Literal and GitHub-supported `github` or `needs` inputs remain valid because this structural policy does not claim transitive information-flow or taint proof through job outputs. Empty permissions leave no configurable repository permission scopes without asserting that no token object exists.
...
- For every reachable local action whose string `runs.using` equals `docker` case-insensitively, accept a string `runs.image` only when it is an existing repository-contained single-link regular Dockerfile whose basename starts `Dockerfile.` or ends `Dockerfile`, case-insensitively, or a full immutable `docker://` digest. Retain an accepted Dockerfile's identity through both final revalidation sweeps. Report missing or escaping Dockerfiles under `local-reference`, unsafe Dockerfile entries under `source-integrity`, and every other string image under `external-image-digest`, without claiming that accepted Dockerfile contents or `FROM` chains are inspected or proven.
...
- Allow repository-local `$/` and `./` reusable-workflow callers to forward named secrets or use `secrets: inherit`; recursive inspection enforces the environment on actual credential-consuming runners in the called workflow. Reject direct secret-context expressions in any reusable caller's `with` inputs under `credential-forwarding`, forcing local credentials through the inspectable `secrets` channel. Preserve literal and GitHub-supported `github` or `needs` inputs without claiming transitive information-flow or taint proof through job outputs. Require exact `jobs.<id>.permissions: {}` on an external reusable-workflow caller, reject any caller `secrets` property at `jobs.<id>.secrets`, and preserve external pinning checks under `external-reference-sha`.
...
Expected: direct development tooling additions are exact `@types/bun@1.3.1` and exact `yaml@2.9.0`. YAML was already resolved transitively, so its direct pin adds only the root dependency edge; Bun typings add the development-only `@types/bun`, `bun-types`, `@types/react`, and `csstype` lock entries. There is no runtime dependency or generated-version change. The branch otherwise contains only the approved programme spec, MAR-2574 plan, policy/tests, workflow/Dependabot/package-script configuration, and maintained documentation.
...
- The exact current-main-integrated MAR-2574 code and behavioural-test implementation snapshot is `7afd091af302f14200637f8798ed3b693a126c87`. It preserves the current release-equivalent gate and trusted retained-artifact flow, runs the workflow-policy test and checker directly through Node after a frozen lifecycle-disabled install, and proves that package lifecycle hooks and Bun preloads cannot repair unsafe workflow source before authoritative validation. The hostile package-script control derives a canonical relative path with portable separators and quoting so the same single checker runs on Windows and POSIX hosts. It retains present, missing, and invalid contained Dockerfile observations through both final sweeps. It also closes the Windows owned-directory path-observation race exposed by exact-head CI: a native-realpath failure is reclassified as path churn only after the captured directory disappears or changes identity, while the original operational error is preserved when identity remains stable. The maintained contract and programme design carry this exact snapshot; the documentation-only provenance commit does not alter runtime or workflow behaviour.
- At `7afd091af302f14200637f8798ed3b693a126c87`, 67 focused workflow-policy tests passed with one host-inapplicable Windows-only skip and zero failures on current Node and on the CI-pinned Node 24.15.0 with Bun 1.3.1; the independent CI bootstrap suite added three passes. Hosted run `32946904726` exposed and isolated the earlier control fixture's Windows absolute-path quoting defect without finding a policy failure; the portable-path remediation then passed focused Luna review. Later exact-head run `32948741453` exposed a Windows `EBADF` race after one stale-lock contender replaced the recovery directory; the identity-loss remediation and its removal, replacement, and stable-error controls passed focused Luna correctness, concurrency, test, and maintainability follow-up with no findings. The repository policy CLI returned no findings, lint checked 141 files with no fixes, all four TypeScript projects passed, and the full suite reported 769 tests with 766 passed, three established capability skips, and zero failures. Frozen lifecycle-disabled installation checked 39 installs across 66 packages without changes. Benchmarks, build, generated-version validation, package validation, the expected already-published-version publish contract, Encephalon validation of 39 records, and diff hygiene all passed locally before this provenance-only commit.
- Earlier implementation lineage includes the structural-policy commit `16266f47db0c802d818d85db139472616aa930e7`, core workflow snapshot `a2bb85a7b28aefedbcb13b4c61d16bbce3f76c57`, and successive branch, cross-platform, input-integrity, and executable-schema remediations through `a8367f56ccab83755ef27e7636de2efc94f70538`. The pre-integration remediation sequence is `f0f625bfdf43ba69925087b53b93d913c67403bf` for reusable-caller credential semantics, `8dd6d14fe43731276f8f34293adbcec19bc3f7da` for role-keyed traversal and final source-witness revalidation, `006c3101a00b6c27d2191cd7acdcefca69d23270` for exact empty external caller permissions, the accurate external-reference taxonomy, and actionable diagnostics, `355ef8980b3d80c5a861da54c018e68019ee2add` for bounded source validation and path-observation race closure, `c664b733b27efc0ba17fea79c2de5a1ab8aa921c` for exact descriptor byte ceilings, `69e649ecec2cb24528bc56bd5ddaaeb185795f6f` for immutable Docker references, least-authority permission compatibility, and location-aware guidance, `62d145a67dd74254e139ee9e4ab38e53719dcd76` for strict YAML 1.2 parsing and nested workflow-step traversal, `3a3d272b695bb9c8784a4e26c26e25ed21ec0159` for the shared parsed-tree budget, `d3610e0d8c00c63f9456c32e18d03c965d58cf04` for the callback-free second witness sweep, `8e0a0a43fc871397e8c72e3d41e50e5139665853` for exact limits, Bun alignment, and structural release-comment assertions, `d6c6b5fdecd759de497a7a7cd8ded5a0f46be5c7` for nonblocking source opens and final behavioural coverage, `6c37cf05624ac1a93a0dc025a63fb30d3bac7963` for the independent secret-tree budget boundary proof, `1a9e7a0985fc8d5700a31476ec4b4489d54de298` for the explicit cross-platform chain-test timeout, `5955831b42a3ac16fbe89c4480d3452c83bcaae3` for closing the reviewed reusable-input and mutable-image bypasses, `07041dc12e7c2eab89aa16f357621e7e8639e89a` for preserving valid reusable inputs and GitHub-runner-recognised local Dockerfiles while making credential remediation exact, `68cfcb9210c984e864505c37bfef1973164bbf40` for case-insensitive Docker action detection plus repository-contained Dockerfile identity binding, and `7666139ad3d1f4853cfee8710ddc841dc3978aec` for deterministic repository-contained Dockerfile diagnostics and focused unsafe-target coverage. That SHA remains a reviewed pre-current-main milestone; the exact current-main-integrated implementation snapshot is recorded above.
- At `7666139ad3d1f4853cfee8710ddc841dc3978aec`, 63 focused workflow-policy tests passed with one host-inapplicable Windows-only skip and zero failures. The repository policy CLI returned no findings, lint checked 117 files with no fixes, all four TypeScript projects passed, and the full suite reported 564 tests with 562 passed, two established capability skips, and zero failures. The benchmark, build, package, publish-contract, frozen-install, and workflow gates passed before the provenance-only commit. Encephalon validation accepted all 38 records, and diff hygiene returned no findings. Pull-request CI run `32680539447` is green evidence for the preceding `5d9f77966092747c40389ad8421325410b0a9dd2` snapshot, including the corrected Windows timeout, but predates the final bot-review remediation.
...
- The local repository work does not complete the external rollout. The ticket branch is pushed and draft pull request #66 exists. The live `pullfrog-review` environment now exists, but it has no protection rules or deployment branch policy; repository `sha_pinning_required` remains false, and no accepted protected Pullfrog dispatch, Linear completion, or merge has occurred. Both the pinned v0.1.60 release and the latest audited upstream v0.1.64 still mint an internal MCP token with `contents: write` despite `push: disabled`, and their later exact-version agent-runtime production dependencies still resolve outside the action lock; both remain upstream MAR-2574 acceptance blockers.
...
3. Protect the existing `pullfrog-review` GitHub environment with an independent required reviewer and self-review prevention. Use custom deployment branch policies for exact `main` and exact `mar-2574-ci-pin-secret-bearing-actions-and-minimize-workflow`; never leave the environment unrestricted.
...
9. Leave the pull request unmerged until the upstream token and dependency-closure blockers are closed and every exact-head gate passes.
