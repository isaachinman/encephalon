...
- Reclaim only exact stable evidence for a positively dead canonical owner or unchanged missing/malformed owner aged strictly more than 5,000 ms; preserve oversized owners without exact bounded bytes.
...
**Interfaces:**
- Add maintenance-only observation bracketed by cache-location checks; its caller preserves unsupported or failed candidate observations only after revalidating the current fixed lock and cache authority.
...
- [x] **Step 3: Apply the grace policy.** Missing owner uses directory mtime; malformed stable single-link owner uses the newer directory/file mtime; require age strictly greater than 5,000 ms. Preserve oversized owners without exact bounded bytes, recovery witnesses, hard links, unsupported types, unreadable evidence, and extra children.
