...
Guarded identity decisions preserve device and inode as `bigint` and stable timestamps at nanosecond precision. The shared comparators deliberately distinguish complete stable reads, identity-only parent-directory instance checks after an independent directory-type proof, and post-rename publication checks that omit only the expected `ctimeNs` change. Cache-manifest projection shares the same authority but retains its exact decimal-string fields and byte representation. This is an internal, backwards-compatible correction with no public API, canonical JSON, cache schema, manifest, instruction, or other stored-format migration.

The exact reviewed MAR-2573 code and behavioural-test snapshot is `849542580e4caa4746bbeed5523a0af65a9df63e`. It centralises lossless identity and manifest projections, retains instruction-specific permission policy without synthetic metadata, rejects same-inode record mutation across pathname and descriptor observations, and keeps Node-only package consumers free of private `node:fs` declaration dependencies. Public APIs, errors, canonical JSON, cache schema, manifests, instructions, and other stored formats remain unchanged.
