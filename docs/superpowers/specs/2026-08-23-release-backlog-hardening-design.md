...
Pullfrog continues using its hosted OIDC mode. Empty provider-secret mappings are removed, `id-token: write` remains narrowly scoped to the pinned Pullfrog job, checkout credentials are not persisted, and the job uses a protected `pullfrog-review` environment. Repository SHA-pin enforcement is enabled after the pinned workflow is present.
