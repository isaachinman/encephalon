...
Recursive visits are keyed by comparable source path and workflow/action role, allowing one dual-role source to be inspected once in each role while cycles terminate. Policy acceptance retains bounded file and action-directory witnesses, then revalidates repository/workflow discovery, every successfully read source, and every exact action-manifest candidate set at one final boundary. Any late instability discards provisional findings and returns a deterministic `source-integrity` result without retrying traversal. Invalid discovery, malformed or unsafe sources, and ambiguous action manifests use the same taxonomy; `local-reference` is reserved for disallowed, escaping, or missing local targets.

Workflow job policy distinguishes reusable-workflow callers from runners. Runner jobs with secret-context expressions, workflow-level secret-bearing env, or effective OIDC authority require the exact `pullfrog-review` environment. A local reusable-workflow caller may forward named secrets or use `secrets: inherit` only because the called workflow is recursively inspected and its credential-consuming runners must independently target that environment; caller workflow env is not propagated. A pinned external reusable workflow must declare exact `jobs.<id>.permissions: {}` and omit `secrets`, leaving no configurable repository permission scopes without claiming that no token object exists. Named or inherited external secrets remain rejected under `credential-forwarding`.

Diagnostics keep the `file:location: rule` prefix and append exhaustive remediation: exact protected environment, removal of external secret forwarding, lowercase 40-character pin under `external-reference-sha`, allowed contained local target, exact least-authority permissions, or stable unambiguous sources for `source-integrity`.
...
The preceding repository-controlled MAR-2574 implementation and behavioural-test snapshot is `a8367f56ccab83755ef27e7636de2efc94f70538`. It uses official exact `@types/bun@1.3.1` declarations only as a development dependency, with Bun types and `skipLibCheck` limited to the scripts project and the historical handwritten Bun declaration removed; Node consumers and the runtime package remain unchanged. It recursively enforces immutable external action references, exact workflow and job permission maps, exact environment targeting for runner jobs with direct or inherited OIDC authority, stable repository-contained workflow discovery, and policy findings rather than filesystem exceptions for invalid repository roots; preserves platform-correct path comparison, Windows junction fixtures, equivalent parser trigger keys, portable POSIX-only fixture handling, and stricter POSIX path semantics; rejects multiply linked workflow inputs during descriptor-bound reads; limits `uses` enforcement to executable job and step positions; requires commit-bound `$/` references for repository-owned action execution while retaining both valid same-commit forms for reusable workflows; binds reviewed adjacent version comments to the exact checked-in workflow pins in behavioural coverage; disables checkout credentials and git pushes; and forces the pinned local Pullfrog core so v0.1.60 cannot bootstrap mutable `pullfrog@^0.1.60`. It does not claim complete upstream immutability or MAR-2574 acceptance: v0.1.60 still mints an internal MCP token with `contents: write` despite `push: disabled`, and its later exact-version agent-runtime production dependencies resolve outside the action lock. Both require an upstream least-authority release before the external environment rollout can complete acceptance.
