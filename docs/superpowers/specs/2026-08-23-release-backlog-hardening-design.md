...
MAR-2574 establishes immutable action identities, structural workflow policy, and the protected Pullfrog trust boundary before MAR-2640 makes those workflows mandatory. CI policy remains dependency-free and recursively inspects repository-local reusable workflows and composite actions so local wrappers cannot hide mutable external actions.

Pullfrog continues using its hosted OIDC mode. Provider-secret mappings are removed, `id-token: write` remains narrowly scoped to the pinned Pullfrog job, checkout credentials are not persisted, pushes are disabled, and the workflow targets the `pullfrog-review` environment. The live environment is currently absent. Creating and protecting it for exact `main` and the exact ticket branch under review, then validating hosted OIDC, remains a planned external rollout and has not yet been applied. Repository `sha_pinning_required` remains false and is enabled only after the pinned workflow is present on `main`.

The repository-controlled MAR-2574 implementation and behavioural-test snapshot is `a2bb85a7b28aefedbcb13b4c61d16bbce3f76c57`. It uses official exact `@types/bun@1.3.1` declarations only as a development dependency, with Bun types and `skipLibCheck` limited to the scripts project and the historical handwritten Bun declaration removed; Node consumers and the runtime package remain unchanged. It recursively enforces immutable external action references, disables checkout credentials and git pushes, and forces the pinned local Pullfrog core so v0.1.60 cannot bootstrap mutable `pullfrog@^0.1.60`. It does not claim complete upstream immutability or MAR-2574 acceptance: v0.1.60 still mints an internal MCP token with `contents: write` despite `push: disabled`, and its later exact-version agent-runtime production dependencies resolve outside the action lock. Both require an upstream least-authority release before the external environment rollout can complete acceptance. This local snapshot includes no push, pull request, dispatch, Linear mutation, or GitHub settings change.
...
- create and protect the `pullfrog-review` environment;
- enable repository action SHA-pin enforcement;
- dispatch one approved Pullfrog validation because the environment changes the OIDC subject.
