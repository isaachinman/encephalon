...
Last reviewed: 2026-08-12 for audited snapshot `ea5c6a7e3b96a7e0a3da8a4c8b03a5e890584d79`.
...
- Before canonical publication, record addition and initialisation stream at most 1,001 direct `_staging` entries and clean at most 1,000. The all-or-nothing preflight accepts only regular files, hard-link aliases, and symlinks whose names exactly match Encephalon's `record-<pid>-<UUID>.tmp` format. Overflow, unrecognised names, unsupported types, late entries, and generation or entry replacements preserve remaining entries and fail with a repository-relative issue and manual recovery instruction; errors never disclose arbitrary entry names. Cleanup performs no recursive scan or removal and never follows symlinks. It atomically moves each accepted entry to a unique sibling quarantine, binds regular entries to no-follow descriptors, verifies the quarantined identity immediately before unlink, and flushes the verified staging directory before canonical publication even when no stale entries were found, where directory flushing is supported.
- Publication keeps the unique current-operation staging descriptor open through bounded staging inspection, canonical linking, and verification that both pathnames still identify the descriptor. Its post-publication cleanup is restricted to that exact owned name and captured identity; it never broad-cleans late arrivals. Post-publication cleanup or flush failures retain the structured committed-error contract.
...
- Manifest entry metadata comes from one non-following filesystem inspection within captured real-parent generations. Root and kind witnesses surround enumerated entry inspection, while referenced artifacts additionally witness each real directory from `encephalon` through the artifact parent. Enumerated disappearance, link substitution, or parent-generation change is retried as a repository change; symbolic links remain symbolic links and are not followed for manifest metadata, while genuine operational filesystem failures retain their I/O classification.
...
- MAR-2569 bounded, ownership-aware staging recovery and behavioural coverage: `504c530639bd6484776b084e79f6fa76e9faa063`.
