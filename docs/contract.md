...
Last reviewed: 2026-08-12 for audited snapshot `4d37a7c8a15b7e734a0c00a0690705d808f2dac8`.
...
- Cleanup performs one bounded initial enumeration and, after all planned removals, a final probe that reads at most one entry. It does not rescan the remaining set after every unlink. Overflow, malformed or unrecognised names, unsupported types, late entries, and generation or entry replacements preserve the affected entries and fail with repository-relative inspect-and-retry guidance; errors never disclose arbitrary entry names. Partial stale-file cleanup may remain visible after a concurrent change, and a later retry safely reclassifies the remaining bounded set.
- Publication keeps the unique current-operation staging descriptor open through bounded staging inspection, canonical linking, staging cleanup, directory flushes, publication-authority acceptance, and a final canonical identity check. The canonical pathname is verified against the descriptor both before and after authority acceptance. A late staging child or lost pathname generation after linking is reported as a structured committed repository change rather than success; genuine operational I/O failures retain the committed I/O classification. Post-publication cleanup is restricted to the exact current owned name and captured identity and never broad-cleans late arrivals.
...
- MAR-2569 bounded, ownership-aware staging recovery and final publication-identity coverage: `4d37a7c8a15b7e734a0c00a0690705d808f2dac8`.
