...
- Before canonical publication, record addition and initialisation stream at most 1,001 direct `_staging` entries and clean at most 1,000. The all-or-nothing preflight accepts only regular files, hard-link aliases, and symlinks whose names exactly match Encephalon's `record-<pid>-<UUID>.tmp` format. Overflow, unrecognised names, unsupported types, and generation or entry replacements preserve remaining entries and fail with a repository-relative issue and manual recovery instruction; errors never disclose arbitrary entry names. Cleanup performs no recursive scan or removal, never follows symlinks, verifies each destructive boundary, and flushes the verified staging directory before canonical publication where directory flushing is supported.
- Publication removes the unique staging file created by the current operation after revalidating its exact root and staging generations. Post-publication cleanup or flush failures retain the structured committed-error contract.
...
- MAR-2569 bounded, ownership-aware staging recovery and behavioural coverage: `ea5c6a7e3b96a7e0a3da8a4c8b03a5e890584d79`.
