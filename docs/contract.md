...
- Artifact syntax remains a schema concern. Filesystem inspection captures the real `encephalon` root once per batch, binds each real non-link ancestor to its verified parent generation, and opens each final path read-only with no-follow semantics where the runtime provides them. Initial `lstat`, descriptor `fstat`, final descriptor `fstat`, final path `lstat`, and ancestor revalidation must agree on complete stable metadata before an immutable artifact observation is accepted.
- Record validation performs that artifact inspection once and hands the verified observations directly to cache-manifest construction; cache rebuild does not restat detached artifact paths. Freshness checks necessarily re-inspect the cached artifact path set through the same primitive. A statically missing, linked, or non-regular artifact is an invalid record artifact; replacement or mutation during inspection is a repository change subject to the bounded rebuild retry policy; genuine operational filesystem failures retain their I/O classification. Artifact errors do not expose absolute paths.
