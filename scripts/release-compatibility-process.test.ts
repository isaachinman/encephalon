...
const standInIndex = (version: string, schemaVersion: string, behaviour = version, mutationTarget?: string) => {
  const fullMaximum = 1000
  const compactMaximum = 1000
...
describe('release compatibility process fixture', () => {
  ...
  test('upgrades and downgrades supplied local package bytes in fresh processes without changing durable state', {
    ...
    try {
      ...
      const oracle = buildStandInTarball(temporaryRoot, '0.3.0', '2')
      ...
      const report = runReleaseCompatibility({
        ...
        oracle: {
          identity: {
            ...
            specifier: 'local-encephalon@0.3.0',
          },
      ...
      assert.deepEqual(report.upgrade.resultLimits.api, expectedLimits)
      assert.deepEqual(report.upgrade.resultLimits.cli, expectedLimits)
      assert.deepEqual(report.downgrade.resultLimits.api, expectedLimits)
      assert.deepEqual(report.downgrade.resultLimits.cli, expectedLimits)
...
describe('release compatibility process fixture group A preloads', () => {
  test('removes preload variables from npm and every installed-package child process', {
    ...
    try {
      mkdirSync(fixtureRoot)
      const oracle = buildStandInTarball(temporaryRoot, '0.3.0-environment-witness', '2')
