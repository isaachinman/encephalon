...
const packageTests = 'test/package.test.ts'
...
if (group === 'main' || group === 'all' || group === 'package' || pattern) {
  const files =
    group === 'main' || group === 'all'
      ? ['scripts', 'test']
          ...
          .filter(path => group === 'all' || (path !== compatibility && path !== packageTests))
      : [group === 'package' ? packageTests : compatibility]
  ...
  throw new Error('Expected all, main, package, compatibility-a, compatibility-b, or compatibility-c test group.')
}
