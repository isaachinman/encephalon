...
CI and Pullfrog external actions and reusable workflows are identified by reviewed immutable commit SHAs with adjacent release comments. Package assertions and normal dependency review maintain those comments because the structural YAML parser does not consume comments. `bun run check:workflows` parses executable workflow and composite-action positions and recursively follows repository-local reusable workflows and composite actions, so an input merely named `uses` remains data and a local wrapper cannot conceal a mutable external reference. Repository-owned action calls use the commit-bound `$/` self-repository form; workspace-relative `./` action calls are rejected, while same-repository reusable workflows retain both valid same-commit forms. Local reusable calls may map or inherit secrets only because their called workflows are recursively inspected. External reusable calls must declare exact empty job permissions and no explicit secrets, leaving them no configurable repository permission scopes without claiming that a token object does not exist. The workflows otherwise keep repository permissions read-only, disable checkout credential persistence, and map no provider secrets to Pullfrog. Pullfrog targets the `pullfrog-review` environment and uses hosted OIDC, but the checked-in target does not provision or protect that GitHub environment. Before any dispatch, maintainers must create and protect it, require approval, and allow only exact `main` plus the exact ticket branch currently under review. External reference SHA enforcement must be enabled only after the pinned workflow reaches `main`.

Workflow diagnostics retain the `file:location: rule` prefix and append concise remediation: `credential-environment` targets exact `pullfrog-review`; `credential-forwarding` removes external-call secrets; `external-reference-sha` pins a lowercase 40-character commit SHA; `local-reference` selects an allowed repository-contained target; `permission` applies the exact least-authority map; and `source-integrity` restores stable, unambiguous regular sources.
