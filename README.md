...
Unrelated malformed, linked, unsupported, unreadable, or changing candidate-shaped entries are inert. Their local maintenance failures are best effort only while the cache location and current fixed lock remain exact. Reclamation requires a real contained candidate directory, unchanged exact directory/owner/recovery-witness observations, and an exact child set of either empty or only `owner.json`. A canonical owner is immediately eligible only when its token matches the directory UUID and its PID is positively absent with `ESRCH`; live, reused, permission-denied, and uncertain PIDs are preserved. Missing, malformed, or oversized supported owner evidence requires unchanged age strictly greater than 5,000 ms. Links, recovery witnesses, ambiguous evidence, and extra children are preserved. The fixed lock and recovery marker, current candidate, cache location, and SQLite gate retain fail-closed handling. Public APIs, operation results, errors and their precedence, cache schema, canonical data, and the single 60-second acquisition deadline are unchanged.
