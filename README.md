...
Local reusable calls may map or inherit secrets only because their called workflows are recursively inspected. Secret-context expressions are rejected in reusable-call `with` inputs; local credentials must use the `secrets` channel so the called workflow's credential-consuming runners remain visible to policy, while external calls may receive no repository credentials. Top-level workflow permissions must be explicit and may be exactly `{}` or `{ contents: read }`. Ordinary runner and repository-local caller jobs may inherit by omission or use either exact map; `{}` overrides inherited authority but does not relax the protected-environment requirement for secret-bearing runners. This is a monotonic lower-authority compatibility broadening: top-level omission and write/additional scopes remain rejected. External reusable calls must declare exact empty job permissions and no explicit secrets, leaving them no configurable repository permission scopes without claiming that no token object exists. The checked-in workflows otherwise keep repository permissions read-only, disable checkout credential persistence, and map no provider secrets to Pullfrog. Pullfrog targets the `pullfrog-review` environment and uses hosted OIDC, but the checked-in target does not provision or protect that GitHub environment. Before any dispatch, maintainers must create and protect it, require approval, and allow only exact `main` plus the exact ticket branch currently under review. Repository action SHA enforcement must be enabled only after the pinned workflow reaches `main`.

Workflow diagnostics retain the `file:location: rule` prefix and append concise remediation: `credential-environment` targets exact `pullfrog-review`; `credential-forwarding` moves local credentials from `with` to `secrets` or removes external-call credentials; `external-image-digest` pins a lowercase 64-character SHA-256 digest; `external-reference-sha` pins a lowercase 40-character commit SHA; `local-reference` selects an allowed repository-contained target; `permission` names the literal maps accepted at that workflow, ordinary job, ambiguous external-caller, or exact Pullfrog location; and `source-integrity` restores stable, unambiguous regular sources.
