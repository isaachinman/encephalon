...
Repository-controlled policy input is bounded to 256 raw workflow-directory entries, 256 KiB per source, 4 MiB across all sources, 512 unique source-path/role visits, and 16,384 parsed secret-tree nodes. Sources are read through bounded descriptors, local traversal uses a deterministic iterative queue, and secret-tree cycles are rejected. Exceeding any bound discards provisional diagnostics and returns the single global `source-integrity` finding.
