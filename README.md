...
Managed instruction replacements bind the repository root, flushed staged file, and any predecessor to no-follow descriptors. An existing predecessor is moved with exclusive hard-link-and-unlink steps: the new backup alias is verified and the directory is flushed before the canonical alias is removed. The replacement commits when the staged file is hard-linked at the canonical `AGENTS.md` or `CLAUDE.md` path, and that publication is flushed before recovery aliases are removed. A successful replacement leaves the canonical file and no temporary, backup, or deletion alias created by that operation; generated-looking historical files are not discovered or removed. Failures after the canonical hard link preserve the committed target and return safe structured details naming the primary phase plus a bounded `postCommitFailures` list of every distinct failed or deferred recovery phase. A later successful cumulative directory flush supersedes an earlier transient publication-flush failure. Retrying converges on the one existing managed block. Recovery copies are private until their exact bytes and preserved final mode are installed. Node has no portable descriptor-relative conditional unlink, so a narrow same-user replacement window remains between the final identity check and pathname unlink; directory durability is best effort where directory fsync is unsupported.
