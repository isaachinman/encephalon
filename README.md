...
Canonical reads and validation accept only one proven-stable generation. Acceptance rechecks the exact bounded root and kind entry sets, including names and types, each opened record's stable metadata and raw-byte digest, and referenced artifact evidence after graph validation. A one-off change discards the complete attempt and returns only the settled generation. One records-owned ledger permits at most three complete attempts within one non-resetting 60-second deadline; the deadline is checked only before a retry, so a slow stable first attempt can still succeed. A stable invalid repository with no intervening generation change keeps its ordinary validation result. After an observed generation change, cache and read operations and no-add initialisation return the settled successor's ordinary validation result; record-producing add or initialisation instead retains its path-free `REPOSITORY_CHANGED` mutation classification when that successor is invalid. Repeated churn ends with the path-free `REPOSITORY_CHANGED` message `The canonical repository changed repeatedly during the operation.`

Add and record-producing initialisation may replan only before their first canonical hard link. A safe retry recomputes candidate validation, `createdAt`, layout additions, and publication authority from the settled generation. After any record links, no canonical retry is permitted. A detected committed race reports `REPOSITORY_CHANGED` with `canonicalCommitted: true`, `repositoryChanged: true`, a bounded frozen `committedRecordIds` list, `postCommitPhase: 'publicationVerification'`, and the fixed recovery action `Run validate and reconcile the canonical repository before retrying the operation.` Add retains its existing `recordId` and repository-relative `path` fields. For a mid-batch or pre-next-link initialisation race, initialisation reports the exact committed prefix at both the top level and in `initProgress`, publishes no later record, and performs no cache or instruction work. A cache-phase generation change may begin cache DML, but its transaction rolls back without mixed rows or instruction work. Follow the reported recovery action before repeating the operation.
...
The disposable cache lives at `node_modules/.cache/encephalon/brain.sqlite` and should not be committed. Encephalon verifies that its cache ancestors, SQLite files, sidecars, and operation-lock entries are real contained entries before use and identity-specific cleanup. Mutable SQLite databases and sidecars must also have exactly one hard link; canonical record publication uses a separate guarded hard-link contract. Recovery markers use immutable `recovering` owner metadata plus an exact durable `recovered` witness, allowing any process to reclaim completed cleanup debt without age-breaking a live active recovery; phase-less legacy owners are treated as recovering. After a gate transaction begins, Encephalon closes SQLite only when cache containment and every sidecar pathname are positively proven safe. Otherwise it preserves the current pathnames by failing closed without closing that connection and rejects further opens for the affected database in the current process. Correct the unsafe cache layout and restart the process before retrying. Encephalon uses SQLite WAL mode, FTS5, a repository-scoped operation lock, manifest-based freshness, and transactional table rebuilds. Canonical records remain the source of truth.

The authoritative operation lock is the SQLite `BEGIN IMMEDIATE` gate transaction. Before taking that gate, an operation does exact work only on its own random `operation.lock.<uuid>` candidate and canonical three-field `{ acquiredAt, pid, token }` owner. After the gate succeeds, the operation promotes that exact candidate to the fixed `operation.lock`, then performs candidate maintenance while the gate is still held and before running the protected callback. One maintenance call visits at most 64 raw cache-directory entries, fully inspects at most 16 canonical candidate names, and attempts at most 4 reclamations; all names, including unrelated ones, count towards the 64-entry budget. At most 8 path-and-identity-bound directory cursors are retained privately so repeated operations in the same process can resume a pass. Cursor state is not persisted: a process restart may restart discovery from the beginning, and no cross-process progress or fairness is promised.

Unrelated malformed, linked, unsupported, unreadable, or changing candidate-shaped entries are inert. Their local maintenance failures are best effort only while the cache location and current fixed lock remain exact. Reclamation requires a real contained candidate directory, unchanged exact directory/owner/recovery-witness observations, and an exact child set of either empty or only `owner.json`. A canonical owner is immediately eligible only when its token matches the directory UUID and its PID is positively absent with `ESRCH`; live, reused, permission-denied, and uncertain PIDs are preserved. Missing or malformed supported owner evidence requires unchanged age strictly greater than 5,000 ms. Oversized owners, links, recovery witnesses, ambiguous evidence, and extra children are preserved because maintenance has no exact bounded bytes for oversized evidence. The fixed lock and recovery marker, current candidate, cache location, and SQLite gate retain fail-closed handling. Public APIs, operation results, errors and their precedence, cache schema, canonical data, and the operation lock's existing non-resetting 60-second acquisition deadline are unchanged. That acquisition deadline is separate from the canonical retry ledger's 60-second deadline.

Cache rebuilds, including automatic preparation, forced hydration, post-add hydration, initialisation, and disposable-corruption recovery, consume the same sealed records-owned snapshot and exact manifest as canonical validation. The bounded optimistic manifest probe remains only a pre-lock freshness decision; after validation begins, the cache does not run a parallel full-corpus scanner or reset the canonical retry ledger. The same snapshot is asserted immediately before cache DML and immediately before `COMMIT`, so a changed generation rolls back without mixed rows or quarantine of an otherwise valid cache. Pre-commit, read, and no-add work may retry only within the one shared three-attempt/60-second ledger. After a canonical record commit, any required snapshot reacquisition is one-shot, bracketed by the exact committed generation, and cannot adopt a predecessor or successor. The cache writer also retains its claimed database identity across retry and recovery.

This stable-generation boundary changes no public API or successful result shape, canonical JSON or artifact layout, package export, runtime dependency, cache schema version `1`, manifest JSON/hash representation, logical SQLite rows or FTS projection. Existing repositories and caches require no migration.
...
CI and Pullfrog owner/repository external actions and reusable workflows are identified by reviewed immutable commit SHAs with adjacent release comments. Workflow-policy assertions bind every structurally observed executable reference to its exact YAML scalar comment, while normal dependency review maintains the reviewed reference-to-release map. Executable `docker://` action references require a lowercase 64-character SHA-256 digest. Every reachable local Docker action, including case-insensitive `runs.using` spellings accepted by the GitHub runner, must use that immutable form unless `runs.image` names an existing repository-contained single-link regular Dockerfile whose basename starts `Dockerfile.` or ends `Dockerfile`, case-insensitively. The policy retains that Dockerfile's exact identity through both final revalidation sweeps without reading or proving its contents or `FROM` chain. `bun run check:workflows` parses executable workflow and action positions and recursively follows repository-local reusable workflows and actions, so an input merely named `uses` remains data and a local wrapper cannot conceal a mutable external reference. Workflow runner steps inside nested `parallel` groups are inspected iteratively at their exact locations; composite actions do not support `parallel`, and similarly named data fields remain non-executable. Repository-owned action calls use the commit-bound `$/` self-repository form; workspace-relative `./` action calls are rejected, while same-repository reusable workflows retain both valid same-commit forms.

Policy sources are parsed with exact development-only `yaml@2.9.0` in YAML 1.2 core mode after the 256 KiB source bound. The parser requires one plain-object document, string keys, unique semantic keys, no merges, and zero errors or warnings; quoted/unquoted or escaped-equivalent duplicates, ambiguous boolean/string keys, unknown tags, non-scalar keys, and excessive alias expansion fail as `source-integrity`. Bounded simple aliases remain valid, with `toJS({ maxAliasCount: 100 })` enforcing the alias ceiling.

Local reusable calls may map or inherit secrets only because their called workflows are recursively inspected. Direct secret-context expressions are rejected in reusable-call `with` inputs; local credentials must use the `secrets` channel so the called workflow's credential-consuming runners remain visible to policy, while external calls must omit both `secrets` and direct secret-context inputs. Literal inputs and GitHub-supported `github` or `needs` expressions remain valid; this structural policy does not claim transitive information-flow or taint proof through job outputs. Top-level workflow permissions must be explicit and may be exactly `{}` or `{ contents: read }`. Ordinary runner and repository-local caller jobs may inherit by omission or use either exact map; `{}` overrides inherited authority but does not relax the protected-environment requirement for secret-bearing runners. This is a monotonic lower-authority compatibility broadening: top-level omission and write/additional scopes remain rejected. External reusable calls must declare exact empty job permissions and no explicit secrets, leaving them no configurable repository permission scopes without claiming that no token object exists. The checked-in workflows otherwise keep repository permissions read-only, disable checkout credential persistence, and map no provider secrets to Pullfrog. Pullfrog targets the `pullfrog-review` environment and uses hosted OIDC, but the checked-in target does not provision or protect that GitHub environment. Before any dispatch, maintainers must create and protect it, require approval, and allow only exact `main` plus the exact ticket branch currently under review. Repository action SHA enforcement must be enabled only after the pinned workflow reaches `main`.

Workflow diagnostics retain the `file:location: rule` prefix and append concise remediation: `credential-environment` targets exact `pullfrog-review`; `credential-forwarding` moves local credentials from `with` to `secrets` and tells external calls to omit `secrets` and direct secret-context inputs; `external-image-digest` pins a lowercase 64-character SHA-256 digest; `external-reference-sha` pins a lowercase 40-character commit SHA; `local-reference` selects an allowed repository-contained target; `permission` names the literal maps accepted at that workflow, ordinary job, ambiguous external-caller, or exact Pullfrog location; and `source-integrity` restores stable, unambiguous regular sources.

Repository-controlled policy input is bounded to 256 raw workflow-directory entries, 256 KiB per source, 4 MiB across all sources, 512 unique source-path/role visits, and 16,384 parsed-tree nodes. Sources are read through bounded, no-follow descriptors with nonblocking and no-controlling-terminal flags where supported; local sources and nested parallel steps use deterministic iterative traversal; secret-tree and executable-structure cycles are rejected. Secret scanning and executable-step structural objects share the 16,384 ceiling. Exceeding any bound discards provisional diagnostics and returns the single global `source-integrity` finding.
...
Workflow tooling uses exact development-only `@types/bun@1.3.1` and `yaml@2.9.0`. YAML was already resolved transitively by development tooling, so making it direct adds no resolved package. Bun types and `skipLibCheck` remain limited to the scripts TypeScript project, the historical handwritten Bun declaration remains removed, scripts remain excluded from the package, and the published Node consumer/runtime retains zero dependencies.
