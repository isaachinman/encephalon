...
Disposable-cache validation treats every SQLite value and schema declaration as untrusted. Numeric-only probes run before text transfer and observe at most seven metadata rows or 1,001 record/FTS rows. Bounded PRAGMA probes require the exact owned ordinary-table columns, primary keys, nullability, defaults, index columns/directions, and FTS5 column roles; narrowly normalised owned-SQL checks compare both ordinary-table definitions and the FTS5 declaration with the package's creation authorities. The canonical authorities remain 1 MiB per record and 8 MiB for the 1,000-record corpus; cached `record_json` receives only a fixed 4 KiB-per-record runtime-path allowance, for a derived aggregate ceiling of 12,484,608 bytes (8 MiB + 1,000 × 4 KiB). Because the FTS search document deliberately duplicates searchable record fields such as the payload summary, its derived ceilings are twice those cached-record bounds: 2,105,344 bytes per row and 24,969,216 bytes in aggregate. Schema, metadata, record, and FTS validation, freshness checks, and the requested public read share one verified SQLite transaction, so results cannot mix database generations.
