...
Correctness tests enforce deterministic bounded-work counts without reading production source and include one isolated comparative heap-allocation guard for payload descriptor-map amplification. `benchmark:check` independently owns product timing, RSS, and cache-size ceilings.
