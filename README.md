...
Canonical reads and validation accept only one proven-stable generation. Acceptance rechecks the complete bounded root and kind entry sets, each opened record's metadata and raw-byte digest, invalid or unreadable entries that contributed validation output, and referenced artifact evidence after graph validation. A changed attempt is discarded in full and retried within at most three complete attempts and one non-resetting 60-second deadline. Stable invalid repositories retain their ordinary validation result; persistent churn returns path-free `REPOSITORY_CHANGED`. Existing successful API and CLI shapes, canonical JSON, and cache formats are unchanged.
