...
import { recordProjection } from './record-projection.ts'
...
class CanonicalPreparationChanged extends CanonicalGenerationChanged {}
...
type CanonicalPublicationAuthority = {
  ...
  changed: () => never
  replanAfterPreparation: boolean
  verifiedCorpus: (
    index: RecordCorpusIndex,
    artifacts: readonly ArtifactObservation[],
...
const committedRecordIds = (records: readonly Pick<BrainRecord, 'id'>[]): string[] => {
  const ids = records.slice(0, MAX_CANONICAL_RECORDS).map(record => record.id)
  Object.freeze(ids)
  return ids
}
...
const publicationVerificationError = (record: BrainRecord, cause: unknown) =>
  new EncephalonError(
    ...
    {
      canonicalCommitted: true,
      committedRecordIds: committedRecordIds([record]),
      path: record.path,
      postCommitPhase: 'publicationVerification',
      recordId: record.id,
...
const canonicalRecordCacheManifestEntries = (
  root: string,
  scan: RecordScan,
  observationsByPath: ReadonlyMap<string, RecordObservation>,
    ...
    const children = rootSnapshot.entries
      ...
      .flatMap(kindEntry => {
        ...
        const records = kindSnapshot.entries.map(recordEntry => {
          ...
          const observation = observationsByPath.get(relativePath)
          if (observation === undefined) {
            return fail('REPOSITORY_CHANGED', 'Canonical records changed after validation.')
          }
          return cacheManifestEntry(relativePath, observation.metadata)
        })
        return [
...
type PublicationDirectoryPreparation = 'conflict' | 'created' | 'existing'

const ensurePublicationDirectory = (
  root: string,
  path: string,
  existed: boolean,
  changed: () => never,
): PublicationDirectoryPreparation => {
  if (existed) {
...
const prepareCanonicalPublicationDirectories = (
  root: string,
  kinds: readonly string[],
  authority: CanonicalPublicationAuthority,
  beforeChildPreparation?: () => void,
) => {
  authority.assertCurrent()
  const brainDirectory = resolve(root, 'encephalon')
  const projection = authority.projection()
  const preparedRoot = ensurePublicationDirectory(root, brainDirectory, projection.rootExists, authority.changed)
  if (preparedRoot === 'conflict') {
    return authority.changed()
  }
  if (preparedRoot === 'created' && authority.replanAfterPreparation) {
    throw new CanonicalPreparationChanged()
  }
  if (preparedRoot === 'created') {
    assertPreparedPublicationDirectory(root, brainDirectory, authority.changed)
  }
  beforeChildPreparation?.()
  const preparedKinds = [...new Set(kinds)].map(kind =>
    ensurePublicationDirectory(root, resolve(brainDirectory, kind), projection.rootNames.has(kind), authority.changed),
  )
  const preparedStaging = ensurePublicationDirectory(
    root,
    resolve(brainDirectory, STAGING_DIRECTORY_NAME),
    projection.rootNames.has(STAGING_DIRECTORY_NAME),
    authority.changed,
  )
  if (preparedKinds.includes('conflict') || preparedStaging === 'conflict') {
    return authority.changed()
  }
  const createdChild = preparedKinds.some(prepared => prepared === 'created') || preparedStaging === 'created'
  if (createdChild && authority.replanAfterPreparation) {
    throw new CanonicalPreparationChanged()
  }
  if (createdChild) {
    for (const kind of kinds) {
      assertPreparedPublicationDirectory(root, resolve(brainDirectory, kind), authority.changed)
    }
    assertPreparedPublicationDirectory(root, resolve(brainDirectory, STAGING_DIRECTORY_NAME), authority.changed)
  }
}
...
const parseObservedRecord = (root: string, path: string, bytes: Buffer): BrainRecord => {
  ...
  return Object.freeze({ ...parseRecordFile(parsed, true), path: posixRelative(root, path) })
}
...
const readonlyLookup = <Key, Value>(values: Map<Key, Value>) =>
  Object.freeze({ get: (key: Key) => values.get(key), has: (key: Key) => values.has(key) })

const indexArtifactReferences = (records: readonly BrainRecord[]) => {
  ...
  return {
    artifactCount,
    artifactPaths: Object.freeze([...paths].sort(ordinalStringCompare)),
  }
}
...
const indexRecordCorpus = (records: readonly BrainRecord[], artifacts = indexArtifactReferences(records)) => {
  ...
  for (const record of records) {
    ...
    paths.set(pathKey, record)
    if (idCollision !== undefined) {
      duplicateIssues.push(issue('DUPLICATE_RECORD_ID', `Duplicate record id ${record.id}.`, record.path, record.id))
    }
    ...
    edgeCount += record.supersedes?.length ?? 0
  }
  ...
  return Object.freeze({
    ...
    byId: readonlyLookup(ids),
    duplicateIssues: Object.freeze(duplicateIssues),
    edgeCount,
    records: Object.freeze([...records]),
    supersededIds: Object.freeze({ has: (id: string) => superseded.has(id) }),
  })
}
...
const artifactIssues = (
  root: string,
  index: Pick<RecordCorpusIndex, 'artifactCount' | 'artifactPaths' | 'records'>,
) => {
  const { artifactCount, artifactPaths, records } = index
  if (artifactCount > MAX_ARTIFACT_REFERENCES) {
    return {
      errors: [
        corpusIssue(
...
const validatedArtifactIssues = (
  root: string,
  index: Pick<RecordCorpusIndex, 'artifactCount' | 'artifactPaths' | 'records'>,
  changed?: (() => never) | undefined,
) => {
  try {
    return artifactIssues(root, index)
  } catch (error) {
...
const validateScannedSnapshot = (
  ...
  hooks: RecordReadHooks = {},
  changed?: (() => never) | undefined,
  existingIndex?: RecordCorpusIndex,
): ValidatedRecordScan => {
...
const withCanonicalSnapshotRetry = <Result>(
  ...
  try {
    ...
    if (error instanceof CanonicalGenerationChanged) {
      if (!(error instanceof CanonicalPreparationChanged)) {
...
const readStableCanonicalPlanningScan = (root: string, hooks: RecordReadHooks = {}) =>
  withCanonicalSnapshotRetry(
    () => readCanonicalPlanningScanAttempt(root, hooks),
    createCanonicalSnapshotRetryLedger(hooks.now),
  )
...
const createVerifiedCorpus = (
  ...
  const observations = new Map(
    scan.observations.map(observation => [posixRelative(root, observation.path), observation]),
  )
  const facts = new Map(
    index.records.map(record => {
      const observation = observations.get(record.path)
      if (observation !== undefined) {
        return [
          record,
          Object.freeze({
            bytes: Number(observation.metadata.size),
      ...
      return fail('REPOSITORY_CHANGED', 'Canonical record evidence changed after validation.')
    }),
  )
  const recordsByPath = Object.freeze(
    [...index.records].sort(
      (first, second) => ordinalStringCompare(first.path, second.path) || ordinalStringCompare(first.id, second.id),
    ),
  )
  const recordFacts = (record: BrainRecord) => {
    const accepted = facts.get(record)
    if (accepted !== undefined) {
      return accepted
    }
    return fail('INTERNAL_ERROR', 'The record does not belong to this verified corpus.')
  }
  return Object.freeze({
    ...index,
    artifacts,
    assertCurrent,
    bytes: scan.bytes,
    manifest: canonicalCacheManifest(canonicalRecordCacheManifestEntries(root, scan, observations), artifacts),
    recordFacts,
    recordFingerprint: recordCorpusFingerprint(recordsByPath.map(recordFacts)),
    recordsByPath,
  })
}

/** @internal */
export type VerifiedCorpus = ReturnType<typeof createVerifiedCorpus>
...
const canonicalPublicationAuthority = (
  ...
  const inspectObservation = (observation: RecordObservation): RecordObservation => {
    ...
    if (closeError !== undefined) {
      throw preserveRecordAuthorityError(closeError)
    }
    if (result === undefined) {
      return changed()
    }
  ...
  const authority: CanonicalPublicationAuthority = {
    ...
    verifiedCorpus: (candidate, artifacts, currentRecords = candidate.records) => {
      const records = currentRecords.map((record, position) => {
        if (record === initialRecords[position]) {
          return record
        }
        const { path, ...file } = record
        return Object.freeze({ ...parseRecordFile(file, true), path })
      })
      const index = records.every((record, position) => record === candidate.records[position])
        ? candidate
        : indexRecordCorpus(records)
      const acceptedScan: RecordScan = {
        bytes: observations.reduce((total, observation) => total + Number(observation.metadata.size), 0),
        errors: [],
        layout,
        observations,
      ...
      const assertCurrent = () => {
        ...
        assertCanonicalSnapshotCurrent(root, acceptedScan, artifactEvidence, changed, hooks)
        if (cacheLocation !== undefined) {
          assertCacheLocation(cacheLocation)
        }
      }
      return createVerifiedCorpus(root, acceptedScan, index, artifacts, assertCurrent)
    },
  }
  return authority
}
...
const recordPlanningSnapshot = (
  ...
  const activeHeads = (kind: string, subject: string) => {
    ...
    return planningIndex.activeHeads(kind, subject)
  }
  const authority = () => {
    if (scan.layout === undefined) {
      return changed()
    }
    return canonicalPublicationAuthority(
      root,
      scan.layout,
      scan.observations,
      scan.records,
  ...
  const validateFinal = (
    ...
    const validation = validateScannedSnapshot(
      ...
      hooks,
      changed,
      records.length === scan.records.length && records.every((record, position) => record === scan.records[position])
        ? planningIndex
        : undefined,
    )
...
const rethrowInvalidatedCandidateError = (error: unknown, repositoryChanged: boolean): never => {
        ...
        if (
          ...
          committedError =
            error instanceof CanonicalGenerationChanged
              ? publicationVerificationError(published.record, error)
      ...
      if (committedErrorPhase !== 'publicationFlush' && options.hydrate !== false) {
        try {
          ...
          if (options.cacheLocation === undefined || committedErrorPhase !== undefined) {
            ...
            const corpus = authority.verifiedCorpus(validated.index, validated.artifacts, [
              ...planning.records,
              published.record,
            ])
          ...
          fault(options.hooks, 'after-hydration')
          if (committedErrorPhase === undefined) {
            authority.assertCurrent()
          }
        } catch (error) {
          ...
          if (error instanceof EncephalonError && error.code === 'REPOSITORY_CHANGED') {
            try {
              ...
              if (authorityError instanceof CanonicalGenerationChanged) {
                capturedError = new CanonicalGenerationChanged('Canonical generation changed during cache hydration.', {
                  cause: error,
                })
              }
            }
          }
          capturePostCommitError('cacheHydration', capturedError)
        }
      }
