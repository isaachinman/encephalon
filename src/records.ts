...
const readRecord = (root: string, path: string): BrainRecord => {
  ...
  if (metadata.size > MAX_RECORD_BYTES) {
    return fail("INVALID_ARGUMENT", "Record file exceeds the 1 MiB limit.", { path: relativePath })
  }
            ...
            return readdirSync(kindPath, { withFileTypes: true })
              ...
              .reduce<RecordScan>((kindResult, recordEntry) => {
                ...
                if (
                  ...
                  recordEntry.name.endsWith(".json")
                ) {
                  try {
                ...
                return {
                  ...
                  errors: [
                    ...kindResult.errors,
                    issue("INVALID_RECORD_LAYOUT", "Kind directories may contain only direct regular JSON files.", relativePath),
                  ],
                }
              }, result)
          }
  ...
  const visit = (record: BrainRecord) => {
    if (!visited.has(record.id)) {
      if (visiting.has(record.id)) {
        ...
        ;(record.supersedes ?? []).forEach((targetId) => {
          const target = byId.get(targetId)
          if (target !== undefined) {
            visit(target)
          }
...
const validateScanned = (root: string, scan: RecordScan): ValidateResult => {
  ...
  return { valid: errors.length === 0, recordsChecked: scan.records.length, errors }
}
...
export const addRecordResolved = (
  ...
  if (scan.errors.length > 0) {
    return fail("VALIDATION_FAILED", "Existing canonical records are invalid.", {
      errors: scan.errors.map((error) => ({ code: error.code, message: error.message })),
    })
  }
  const candidate: BrainRecord = { ...recordFile, path: relativePath }
  const candidateResult = validateScanned(root, { records: [...scan.records, candidate], errors: [] })
  if (!candidateResult.valid) {
    return fail("VALIDATION_FAILED", "The new record would make canonical records invalid.", {
      errors: candidateResult.errors.map((error) => ({ code: error.code, message: error.message })),
    })
  }
