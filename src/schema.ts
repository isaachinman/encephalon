...
const requiredText = (value: unknown, field: string) => {
  if (typeof value === "string") {
    if (value.length > 0 && value === value.trim() && byteLength(value) <= MAX_TEXT_BYTES) {
      return value
    }
  }
...
const assertPortableSegment = (value: string, field: string, pattern: RegExp) => {
  ...
  return fail("INVALID_ARGUMENT", `${field} is not a portable path segment.`, { field })
}
...
const validateJsonAt = (value: unknown, seen: WeakSet<object>, path: string): JsonValue => {
  ...
  if (typeof value === "number") {
    ...
    return fail("INVALID_ARGUMENT", "payload contains a non-finite number.", { field: path })
  }
  if (Array.isArray(value)) {
    ...
    if (seen.has(value)) {
      return fail("INVALID_ARGUMENT", "payload contains a cycle.", { field: path })
    }
    seen.add(value)
    const values = Array.from({ length: value.length }, (_, index) => {
      ...
      return fail("INVALID_ARGUMENT", "payload contains a sparse array.", { field: path })
    })
    seen.delete(value)
    return values
  }
  if (typeof value === "object") {
    ...
    return fail("INVALID_ARGUMENT", "payload contains a non-plain object.", { field: path })
  }
...
const portableArtifactSegments = (value: unknown) => {
  ...
  return fail("INVALID_ARGUMENT", "artifact contains an unsafe path segment.", { field: "artifact" })
}

export const validateArtifactPath = (value: unknown, kind: string, id: string) => {
  const artifact = portableArtifactSegments(value)
  if (
    artifact.segments[0] === "_artifacts" &&
...
const normalizeOptionalText = (value: unknown, field: string, maximumBytes: number) => {
  if (typeof value === "string") {
    if (value.length > 0 && value === value.trim() && byteLength(value) <= maximumBytes) {
      return value
    }
  }
  ...
  const supersedes = input.supersedes === undefined
    ...
    : Array.isArray(input.supersedes)
      ? input.supersedes.length === 0
        ? undefined
        : validateStringArray(input.supersedes, "supersedes", validateId)
      : fail("INVALID_ARGUMENT", "supersedes must be an array of strings.", { field: "supersedes" })
    ...
    : Array.isArray(input.artifacts)
      ? input.artifacts.length === 0
        ...
        : input.artifacts.length <= MAX_ARTIFACTS
          ? validateStringArray(input.artifacts, "artifacts", (value) => validateArtifactPath(value, kind, id))
          : fail("INVALID_ARGUMENT", "artifacts may contain at most 256 paths.", { field: "artifacts" })
      : fail("INVALID_ARGUMENT", "artifacts must be an array of strings.", { field: "artifacts" })
  const searchText =
