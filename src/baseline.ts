...
const readPackageFacts = (root: string): PackageFacts => {
  ...
  if (existsSync(path) && lstatSync(path).isFile() && !lstatSync(path).isSymbolicLink() && lstatSync(path).size <= MAX_PACKAGE_BYTES) {
    try {
      ...
      const workspaceValue = Array.isArray(value.workspaces)
        ? value.workspaces
        : value.workspaces !== null && typeof value.workspaces === "object" && "packages" in value.workspaces
          ? (value.workspaces as { packages?: unknown }).packages
          : []
      return {
        ...(typeof value.name === "string" && safeName(value.name) ? { name: value.name } : {}),
        ...(typeof value.packageManager === "string" && PACKAGE_MANAGERS.has(value.packageManager.split("@")[0]?.toLowerCase() ?? "")
      ...
      return { workspacePatterns: [], scriptKeys: [] }
    }
  }
  return { workspacePatterns: [], scriptKeys: [] }
}
...
const scanLanguages = (root: string) => {
  const initial: ScanState = { filesSeen: 0, truncated: false, languageCounts: new Map() }
    ...
    return readdirSync(directory, { withFileTypes: true })
      ...
      .reduce<ScanState>((current, entry) => {
        ...
        if (entry.isFile()) {
          ...
          return { filesSeen, truncated: filesSeen >= MAX_SCANNED_FILES, languageCounts }
        }
        return current
      }, state)
  }
    ...
    .reduce<{ directories: string[]; recognisedFiles: string[] }>(
      (facts, entry) => {
        ...
        if (entry.isFile() && RECOGNISED_FILES.has(entry.name.toLowerCase())) {
          return { ...facts, recognisedFiles: [...facts.recognisedFiles, entry.name] }
        }
        return facts
      },
...
const commandForScript = (manager: string, script: string) => manager === "yarn" ? `yarn ${script}` : `${manager} run ${script}`
...
export const scanBaseline = (root: string): AddRecordInput[] => {
  ...
  const packageManager = packageFacts.packageManager ?? packageManagerFromLock(layout.recognisedFiles) ?? "npm"
  ...
  const safeSources = [...new Set([
    ...layout.recognisedFiles,
    ...(workflows.length === 0 ? [] : [".github/workflows"]),
  ])].sort((first, second) => first.localeCompare(second))
...
const sortJson = (value: JsonValue): JsonValue => {
  ...
  if (value !== null && typeof value === "object") {
    return Object.fromEntries(Object.keys(value).sort((first, second) => first.localeCompare(second)).map((key) => [key, sortJson(value[key] as JsonValue)]))
  }
  return value
}
