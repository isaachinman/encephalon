...
const isRecoverableCacheFailure = (error: unknown) => {
  ...
  return error instanceof CacheSchemaMismatch || sqlite.errcode === 11 || sqlite.errcode === 26 || (
    typeof sqlite.message === "string" &&
    /database disk image is malformed|file is not a database|malformed database schema|no such (?:column|table)|has no column named/i.test(sqlite.message)
  )
}
...
const removeCorruptCache = (root: string) => {
  ...
  ;[path, `${path}-wal`, `${path}-shm`].forEach((candidate) => rmSync(candidate, { force: true }))
}

const verifySQLiteFeatures = (DatabaseSync: SQLiteModule["DatabaseSync"]) => {
  if (sqliteFeaturesVerified) return
  const probe = new DatabaseSync(":memory:")
  try {
    probe.exec("CREATE VIRTUAL TABLE record_search_probe USING fts5(text)")
    probe.prepare("SELECT bm25(record_search_probe), snippet(record_search_probe, 0, '[', ']', '…', 8) FROM record_search_probe WHERE record_search_probe MATCH ?").all("probe")
...
const openDatabase = (root: string) => {
  ...
  verifySQLiteFeatures(DatabaseSync)
  ...
  try {
    ...
    assertTableColumns(database, "records", ["id", "kind", "subject", "source", "created_at", "path", "active", "summary", "record_json"])
    assertTableColumns(database, "record_search", ["id", "text"])
    const searchSchema = database.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'record_search'").get() as { sql?: unknown } | undefined
...
const posixRelative = (root: string, path: string) =>
  path.slice(root.length).replace(/^[/\\]+/, "").replaceAll("\\", "/")
...
const recordManifestEntries = (root: string) => {
  ...
  if (brainEntry.type !== "directory") return [brainEntry]
...
const readMetadata = (database: DatabaseSync): Metadata | undefined => {
  ...
  if (
    ...
    try {
      ...
      if (Array.isArray(artifactPaths) && artifactPaths.every((path) => typeof path === "string") && Number.isInteger(recordsIndexed)) {
        return {
...
const metadataIsFresh = (root: string, metadata: Metadata | undefined) => {
...
const searchDocumentForRecord = (record: BrainRecord) =>
  [record.kind, record.subject, record.source, summaryForRecord(record), JSON.stringify(record.payload), record.searchText ?? ""]
...
const rebuildCache = (root: string): PrepareResult => {
  ...
  for (const attempt of attempts) {
    ...
    const artifactPaths = [...new Set(records.flatMap((record) => record.artifacts ?? []))].sort((first, second) => first.localeCompare(second))
    ...
    try {
      ...
      if (!isRecoverableCacheFailure(error)) throw error
      ...
      if (existsSync(databasePath(root))) {
        ...
        try {
          ...
          if (metadataIsFresh(root, metadata)) {
            return { hydrated: false, recordsIndexed: metadata?.recordsIndexed ?? 0 }
          }
        } finally {
          recheck.close()
        }
...
export const listRecords = (input: ListRecordsInput = {}): BrainRecord[] =>
  withPreparedDatabase(input, (database) => {
    const conditions = [input.includeSuperseded === true ? undefined : "active = 1", input.kind === undefined ? undefined : "kind = ?", input.subject === undefined ? undefined : "subject = ?"].filter((value): value is string => value !== undefined)
    const parameters = [
    ...
    const rows = database.prepare(`SELECT record_json FROM records ${where} ORDER BY created_at DESC, id DESC LIMIT ?`).all(...parameters) as RecordRow[]
...
export const showRecord = (input: ShowRecordInput): BrainRecord | null =>
  withPreparedDatabase(input, (database) => {
    if (typeof input.id !== "string" || input.id.length === 0) {
      return fail("INVALID_ARGUMENT", "id must be a non-empty string.", { field: "id" })
    }
    const activeClause = input.activeOnly === true ? " AND active = 1" : ""
    const row = database.prepare(`SELECT record_json FROM records WHERE id = ?${activeClause}`).get(input.id) as RecordRow | undefined
...
export const gatherRecords = (input: GatherInput): GatherResult => {
  const root = resolveRepository(input)
  try {
    const hydrated = input.hydrate === true
      ? { recordsIndexed: hydrateResolvedRepository(root).recordsIndexed }
      : (prepareResolved(root), null)
    const searches = input.searches ?? []
    const shows = input.shows ?? []
    if (!Array.isArray(searches) || !searches.every((query) => typeof query === "string")) {
      return fail("INVALID_ARGUMENT", "searches must be an array of strings.", { field: "searches" })
    }
    if (!Array.isArray(shows) || !shows.every((id) => typeof id === "string")) {
      return fail("INVALID_ARGUMENT", "shows must be an array of strings.", { field: "shows" })
    }
    const database = openDatabase(root)
    try {
      return {
        hydrated,
        searches: searches.map((query) => ({ query, kind: input.kind ?? null, results: searchRows(database, { ...input, query }).map((row) => ({
          id: row.id,
          kind: row.kind,
          subject: row.subject,
          path: row.path,
        ...
        records: shows.map((id) => {
          ...
          const row = database.prepare(`SELECT record_json FROM records WHERE id = ?${activeClause}`).get(id) as RecordRow | undefined
