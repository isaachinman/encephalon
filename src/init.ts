...
const NEXT_ACTION = "Read ./node_modules/encephalon/skills/encephalon/SKILL.md and perform optional semantic enrichment for durable repository knowledge."
...
const baselineActions = (
  ...
  (result, candidate) => {
    ...
    if (refresh && matching.some((record) => canonicalPayload(record.payload) !== canonicalPayload(candidate.payload))) {
      return {
        ...result,
        additions: [
          ...result.additions,
          { ...candidate, supersedes: matching.map((record) => record.id).sort((first, second) => first.localeCompare(second)) },
        ],
      }
    }
    return result
