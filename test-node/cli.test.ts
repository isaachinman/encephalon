...
const run = (root: string, arguments_: string[]) => spawnSync(
  process.execPath,
  [join(import.meta.dirname, "..", "src", "cli.ts"), ...arguments_],
...
describe("command-line interface", () => {
  test("emits exactly one JSON value for successful commands", () => {
    ...
    const added = run(root, ["--root", root, "add", "--id", "cli-decision", "--kind", "decision", "--subject", "api.style", "--source", "agent", "--data", '{"summary":"Use JSON"}', "--text", "portable output"])
