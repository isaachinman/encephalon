...
`init` safely derives bounded repository metadata, creates up to three baseline records, builds the local cache, and adds a reversible managed block to root `AGENTS.md` and `CLAUDE.md`. For baseline records, it reads only a bounded, verified `package.json` and extracts fields such as package name, package manager, workspace globs, and script keys. It streams at most 513 raw entries from the repository root and `.github/workflows` to distinguish the 512-entry limit from overflow before filtering. It does not traverse source directories or generate language counts or file totals; agents can discover these separately when needed. Workflow directories remain bound to their verified ancestry, and all retained source passes must share one stable repository generation. After assembling the payloads, `init` revalidates every compact source witness together; a changed attempt is discarded in full and retried within at most three complete attempts and one non-resetting 60-second deadline checked before each retry. Persistent churn returns path-free `REPOSITORY_CHANGED`. An overflowed, invalid, or stably unreadable source contributes no rejected facts; the repository overview reports bounded, stable reason codes instead.
...
Compact snippets use a short preview of the record's kind, subject, source, and summary. Payload and `searchText` terms remain searchable; matches outside the preview return a stable fallback. Full search returns complete records. Ranking counts preview occurrences in addition to the complete search body, so metadata and summary matches gain weight and ordering can differ from earlier releases.
...
Canonical directory enumeration is also bounded before record contents are read. The `encephalon` root may contain at most 1,002 entries and 1,000 kind directories, while each kind directory may contain at most 1,000 entries. `_artifacts` and `_staging` count towards the 1,002 root-entry limit but not the kind-directory limit; their reserved directory trees are excluded from the record-manifest portion of cache freshness, while referenced artifact files are included separately. An overflow produces one deterministic `CORPUS_DIRECTORY_ENTRY_LIMIT` issue for the containing repository-relative directory without reporting excess filenames. Validation and cache freshness checks reject directory-generation changes rather than accepting records or metadata from mixed or replaced directories. Record additions carry the validated generation through preflight and an authority check immediately before the canonical hard link. A pre-link canonical change discards the complete add plan and replans within the shared three-attempt, non-resetting 60-second ledger. After the link, success requires the exact accepted generation plus one regular non-symlink record bound to the held staging descriptor; unrelated sibling or retained-evidence changes return committed `REPOSITORY_CHANGED` without retrying or rolling back the append-only record. The error preserves its existing message, recovery action, relative `path`, `recordId`, and `canonicalCommitted` fields, and adds bounded `committedRecordIds` and `repositoryChanged` details. Initialisation derives and validates its complete generated-record batch from one canonical planning snapshot. A canonical change before the first link discards the complete attempt and replans within the same bounded ledger. After the first link, it never replans: each later record and cache completion require the exact accepted generation plus the ordered committed prefix. A change returns committed `REPOSITORY_CHANGED` with the bounded prefix and deterministic validate-and-reconcile guidance before later cache or instruction work; a same-options rerun creates only missing generated records. Before publishing, mutations inspect at most 1,001 direct `_staging` entries and remove up to 1,000 stale files or symlinks only when their names exactly match Encephalon's owned staging or recovery-quarantine formats. Cleanup uses one bounded initial enumeration plus bounded one-entry emptiness probes; it never recursively scans or follows symlinks. Each accepted entry is moved to a random sibling quarantine, identity-verified without following links, and only then unlinked; regular files remain bound to open descriptors. The current operation keeps its staging descriptor through canonical linking, cleanup, directory flushes, authority acceptance, and final canonical and empty-staging verification. Overflow, malformed names or types, late entries, and detected replacements preserve affected entries and return repository-relative inspect-and-retry guidance without disclosing their names. Node has no portable descriptor-relative conditional unlink, leaving a narrow final pathname-syscall window after the adjacent identity checks.

Canonical reads and validation accept only one proven-stable generation. Acceptance rechecks the complete bounded root and kind entry sets, each opened record's metadata and raw-byte digest, invalid or unreadable entries that contributed validation output, and referenced artifact evidence after graph validation. A changed attempt is discarded in full and retried within at most three complete attempts and one non-resetting 60-second deadline. Stable invalid repositories retain their ordinary validation result; persistent churn returns path-free `REPOSITORY_CHANGED`. Existing successful API and CLI shapes and canonical JSON are unchanged.
...
Correctness tests enforce bounded scans, graph traversal and directory reads without inspecting production source or wrapping production collections. One isolated heap test detects excess temporary payload-value retention before rejection. `benchmark:check` independently owns the configured product latency and cache-size ceilings; isolated RSS remains diagnostic unless a budget explicitly selects it.

The full profile runs every operation in fresh child processes with two discarded warmups and five measured samples at 0, 100, and 1,000 records. `benchmark:check` keeps CI to a single 0/100/1,000 sample with generous schema-version 2 p95 and cache-size ceilings. See [docs/performance.md](https://github.com/isaachinman/encephalon/blob/main/docs/performance.md) for phase semantics, memory sources, profiles, budgets, baseline distributions, and scale guidance.
...
Ordinary `bun run test` runs the complete build-independent source suite. `test:tooling` runs cheap tool checks; `test:package` requires an explicit build. `test:history` runs the historical release-tool process regressions, and `test:all` retains all four groups for full local verification after a build. New test files require an explicit coverage classification. macOS and Windows run filesystem, path and process tests, plus changed source-only tests; package consumers exercise installed executable behaviour on all three operating systems.

CI checks source on Linux with Node 24.15.0 and Node 26. After both source lanes, lint, types and cheap tool tests succeed, one package job checks the clean tree before and after the explicit build, requires `package-artifacts` to be absent, and retains one tarball and metadata sidecar. The filename comes from the validated package version. The sidecar records size, SHA-1, SHA-256, SHA-512, npm integrity, source commit and repository-relative path. Consumers download the producer's immutable artifact ID, check its separately recorded SHA-256, and restore only the reviewed `dist` files from that tarball. They never rebuild or repack the candidate.

Actual published `encephalon@0.3.0` upgrade/downgrade compatibility runs for every main candidate and for pull requests touching public, persisted, package or release behaviour. Historical release-tool regressions run when their tools or CI change. The `release` label, a manual main run and the weekly main schedule force both checks. Deletions and both sides of renames participate in selection; unknown paths conservatively require compatibility. Known ordinary documentation or test-only pull requests may skip historical checks. The unchanged six Linux benchmark shards still collect 20 measured samples after two warmups for each operation and side. The exact candidate also receives the benchmark smoke and tarball-only npm publish dry run. No job publishes to npm.

Pull requests retain the auditable candidate. A separate workflow copies a successful main run's verified artifact only after that CI run has completed successfully; its trusted artifact name includes the source commit, originating run and attempt. Superseded PR and main runs are cancelled, and cancelled runs cannot promote a candidate. Consumer-only reruns reuse the original producer artifact; full reruns create an attempt-specific candidate. Before a manual, explicit-approval publish handoff, require the trusted artifact to match current main and compare its bytes with the reviewed PR tarball.

The required branch-protection contexts remain `verify (ubuntu-latest)`, `verify (macos-latest)`, `verify (windows-latest)`, `verify (ubuntu-current)`, `Release-equivalent package gate` and `Greptile Review`. Greptile must report 5/5 for the reviewed head. Maintainers can repeat the read-only verification with:
